...
class AppSettings
	...
	constructor(
		...
		val theme: Int
			get() =
				prefs.getString(KEY_THEME, null)?.toIntOrNull()
		...
		val readerControls: Set<ReaderControl>
			get() =
				prefs.getStringSet(KEY_READER_CONTROLS, null)?.mapNotNullTo(EnumSet.noneOf(ReaderControl::class.java)) {
		...
		var notificationSound: Uri
			get() =
				prefs.getString(KEY_NOTIFICATIONS_SOUND, null)?.toUriOrNull()
		...
		val searchSuggestionTypes: Set<SearchSuggestionType>
			get() =
				prefs.getStringSet(KEY_SEARCH_SUGGESTION_TYPES, null)?.let { stringSet ->
		...
		val periodicalBackupMaxCount: Int
			get() =
				if (prefs.getBoolean(KEY_BACKUP_PERIODICAL_TRIM, true)) {
		...
		fun observe(vararg keys: String): Flow<String?> =
			prefs
				.observeChanges()
