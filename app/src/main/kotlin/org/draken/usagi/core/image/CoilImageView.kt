...
open class CoilImageView
	...
	constructor(
		...
		init {
			context.withStyledAttributes(attrs, R.styleable.CoilImageView, defStyleAttr) {
				...
				crossfadeDurationFactor =
					if (getBoolean(R.styleable.CoilImageView_crossfadeEnabled, true)) {
						crossfadeDurationFactor
					} else {
						0f
					}
		...
		private fun ScaleType.toCoilScale(): Scale =
			if (this == ScaleType.CENTER_CROP) {
				Scale.FILL
			} else {
				Scale.FIT
			}
